...
>("ready");
  ...
  const changeTestThink = useCallback(async (images: any[], promptText: string, onChange: (code: string) => void, setOnClose: (onClose: () => void) => void, onError: () => void) => {
    if (loading) return;
    ...
    try {
      ...
      try {
        await uploadChatLargeData(composerId, {
          text: `${images.map(item => '@' + item.filename).join(' ')}
          ` + promptText,
